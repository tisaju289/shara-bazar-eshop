...
import { SlidersHorizontal, ChevronRight, Home, Tag, X } from "lucide-react";
...
function BrandProductPage() {
  ...
  const catCounts = useMemo(() => {
    ...
    for (const p of allProducts) if (p.category_id) m[p.category_id] = (m[p.category_id] ?? 0) + 1;
    return m;
  }, [allProducts]);
  ...
  }, [allProducts, minPrice, maxPrice, selectedCatId]);
  ...
  const hasFilter = minPrice !== "" || maxPrice !== "" || selectedCatId;
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="container mx-auto px-4 py-4 pb-28 md:pb-12">
        <nav className="flex items-center gap-1.5 text-xs text-muted-foreground mb-4 flex-wrap">
          ...
          <ChevronRight className="size-3.5" />
          <span className="text-foreground font-semibold">{currentBrand?.name_bn ?? "..."}</span>
        </nav>
          ...
          <div className="flex items-center gap-4 mb-6 p-4 rounded-2xl bg-card border border-border">
            {currentBrand.image_url ? (
              <img
                ...
                alt={currentBrand.name_bn}
                className="size-16 md:size-20 rounded-2xl object-contain shrink-0"
            ...
            <div>
              <h1 className="text-xl md:text-2xl font-extrabold text-[var(--leaf-deep)]">{currentBrand.name_bn}</h1>
        ...
        <div className="md:hidden flex items-center gap-3 mb-4">
          ...
          {hasFilter && (
            <button
              onClick={() => { setMinPrice(""); setMaxPrice(""); setSelectedCatId(null); }}
        ...
        <div className="flex gap-6 items-start">
          <aside
            className={`${filterOpen ? "block" : "hidden"} md:block w-full md:w-60 lg:w-64 shrink-0 space-y-4 md:sticky md:top-24`}
          >
            ...
            <div className="bg-card border border-border rounded-2xl p-4">
              <h3 className="font-bold text-sm mb-3">ক্যাটাগরি</h3>
              <div className="space-y-1">
                <button
                  onClick={() => setSelectedCatId(null)}
                  className={`w-full text-left px-3 py-2 rounded-xl text-sm transition flex items-center justify-between ${selectedCatId === null ? "bg-primary text-primary-foreground font-semibold" : "hover:bg-secondary"}`}
                >
                  <span>সব ক্যাটাগরি</span>
                  <span className={`text-xs ${selectedCatId === null ? "opacity-80" : "opacity-60"}`}>{allProducts.length}</span>
                </button>
                {categories
                  ...
                  .map((c) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-xl text-sm transition flex items-center justify-between ${selectedCatId === c.id ? "bg-primary text-primary-foreground font-semibold" : "hover:bg-secondary"}`}
                    >
                      <span className="truncate">{c.name_bn}</span>
                      <span className={`text-xs shrink-0 ml-1 ${selectedCatId === c.id ? "opacity-80" : "opacity-60"}`}>{catCounts[c.id] ?? 0}</span>
                    </button>
                  ))}
              </div>
            </div>
          ...
          <div className="flex-1 min-w-0">
            {isLoading ? (
              ...
              <div className="text-center text-muted-foreground py-24">
                ...
                <p className="text-sm">এই ফিল্টারে কোন পণ্য পাওয়া যায়নি</p>
                {hasFilter && (
                  <button
                    onClick={() => { setMinPrice(""); setMaxPrice(""); setSelectedCatId(null); }}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
                  {filtered.map((p) => (
                    <ProductCard
                      ...
                      brandName={currentBrand?.name_bn ?? ""}
                      qty={cart[p.id] ?? 0}
                      add={add}
                      sub={sub}
